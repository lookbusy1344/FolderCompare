...
/// trait for equality and hashing functions
pub trait Comparer<T> {
    fn equals(&self, a: &T, b: &T) -> bool;
    fn hash(&self, value: &T) -> usize;
}

/// This hashset takes a Comparer trait object to allow different equality and hashing functions
pub struct PluggableHashSet<T, C: Comparer<T>> {
    /// use a vector of vectors as the underlying data structure
    buckets: Vec<Vec<T>>,
    /// store the capacity of the buckets
    buckets_required: usize,
    /// equality checker from trait
    comparer: C,
}

impl<T, C: Comparer<T>> PluggableHashSet<T, C> {
    // Create a new PluggableHashSet with the given equality and hash functions and an initial capacity
    pub fn new(comparer: C, buckets_required: usize, default_bucket_size: usize) -> Self {
    ...
    /// Insert an element into the `PluggableHashSet`. Return true if the element was added
    pub fn insert(&mut self, value: T) -> bool {
        // use the hash function to get the hash of the value
        let hash = self.comparer.hash(&value);
        ...
        if bucket.iter().any(|x| self.comparer.equals(x, &value)) {
    ...
    /// Check if an element is present in the `PluggableHashSet`
    pub fn contains(&self, value: &T) -> bool {
        ...
        bucket.iter().any(|x| self.comparer.equals(x, value))
    }

    /// Remove an element from the `PluggableHashSet`. Returns true if the element was present
    pub fn remove(&mut self, value: &T) -> bool {
        ...
        if let Some(pos) = bucket.iter().position(|x| self.comparer.equals(x, value)) {
    ...
    pub fn rebuild(&mut self, buckets_required: usize, default_bucket_size: usize) {
        ...
        for mut bucket in old_buckets {
            for item in bucket.drain(..) {
                ...
                let hash = self.comparer.hash(&item);
